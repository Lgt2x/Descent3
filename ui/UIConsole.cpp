...
void UIConsole::Create(int x, int y, int font, int cols, int rows)
{
	...
	UIItem item = static_cast<UIItem>(UIPrimativeItem(GR_BLACK));
...
//	functions
void UIConsole::puts(const char *str)
{
	m_Console.puts(GR_GREEN ,str);
}


void UIConsole::puts(ddgr_color col, const char *str)
{
	m_Console.puts(col, str);
}

...
#warning *********************************************************************
#warning *********************************************************************
#define _D(x) DGL_Printf(" // called in Descent 3:\n // "); DGL_Printf x; DGL_Printf("\n\n");
#else
#define _D(x)
#endif
...
module *LoadOpenGLDLL(char *dllname)
{
	...
	dglDisableClientState = dllDisableClientState = (glDisableClientState_fp)mod_GetSymbol(&OpenGLDLLInst,"glDisableClientState",255);
	...
	dglEnableClientState = dllEnableClientState = (glEnableClientState_fp)mod_GetSymbol(&OpenGLDLLInst,"glEnableClientState",255);
	...
	dglTexCoordPointer = dllTexCoordPointer = (glTexCoordPointer_fp)mod_GetSymbol(&OpenGLDLLInst,"glTexCoordPointer",255);
	...
	OpenGL_packed_pixels = false;
        ...
        if (OpenGL_multitexture)
        {
            ...
            /****************** WARNING: NASTY HACK! ***********************/
            	...
            	dglActiveTextureARB = dllActiveTextureARB = (PFNGLACTIVETEXTUREARBPROC) mod_GetSymbol(OpenGLDLLHandle,"glActiveTextureARB",255);
            	if (dglActiveTextureARB == NULL)
		            OpenGL_multitexture=false;

                dglClientActiveTextureARB = dllClientActiveTextureARB = (PFNGLCLIENTACTIVETEXTUREARBPROC) mod_GetSymbol(OpenGLDLLHandle,"glClientActiveTextureARB",255);
            	if (dglClientActiveTextureARB == NULL)
		            OpenGL_multitexture=false;

                dglMultiTexCoord4f = dllMultiTexCoord4f = (PFNGLMULTITEXCOORD4FARBPROC) mod_GetSymbol(OpenGLDLLHandle,"glMultiTexCoord4fARB",255);
...
//
    ...
    mprintf((0, "OpenGL: multitextures are%s turned on.",
                (OpenGL_multitexture) ? "" : " NOT"));

    mprintf((0, "OpenGL: Packed pixels are%s turned on.",
                (OpenGL_packed_pixels) ? "" : " NOT"));

    mprintf((0, "OpenGL: Fog is%s turned on.",
                (OpenGL_fog_enabled) ? "" : " NOT"));
	...
	if (OpenGL_packed_pixels)
	{
		...
		{	
            ...
            if (size > 0)
            {
    			...
    			if (replace)
    			{
    				dglTexSubImage2D (GL_TEXTURE_2D,0,0,0,size,size,GL_BGRA,GL_UNSIGNED_SHORT_1_5_5_5_REV,bm_ptr/*opengl_packed_Upload_data*/);
    			} // if
    			else
                {
        			dglTexImage2D (GL_TEXTURE_2D,0,GL_RGB5_A1,size,size,0,GL_BGRA,GL_UNSIGNED_SHORT_1_5_5_5_REV,bm_ptr/*opengl_packed_Upload_data*/);
                } // else
                #else
    			if (replace)
    			{
		...
		else
		{
			...
			for (int m=0;m<limit;m++)
			{
				...
				if (bm_format(bm_handle)==BITMAP_FORMAT_4444)
				{
					...
					if (replace)
						dglTexSubImage2D (GL_TEXTURE_2D,m,0,0,w,h,GL_BGRA,GL_UNSIGNED_SHORT_4_4_4_4_REV, bm_mipped(bm_handle)? opengl_packed_Upload_data : bm_ptr);
					else
						dglTexImage2D (GL_TEXTURE_2D,m,GL_RGBA4,w,h,0,GL_BGRA,GL_UNSIGNED_SHORT_4_4_4_4_REV, bm_mipped(bm_handle)? opengl_packed_Upload_data : bm_ptr);
				...
				else
				{
					// Do 1555
      				...
      				if (replace)
					  dglTexSubImage2D (GL_TEXTURE_2D,m,0,0,w,h,GL_BGRA,GL_UNSIGNED_SHORT_1_5_5_5_REV,bm_ptr/*opengl_packed_Upload_data*/);
					else
					  dglTexImage2D (GL_TEXTURE_2D,m,GL_RGB5_A1,w,h,0,GL_BGRA,GL_UNSIGNED_SHORT_1_5_5_5_REV,bm_ptr/*opengl_packed_Upload_data*/);
...
static void slownessAbort(void)
{
        ...
        SDL_SetVideoMode(surface->w, surface->h, surface->format->BitsPerPixel,

...
// save original settings.
	...
	{
		Controller->get_controller_function(Cfg_key_elements[i].fn_id, m_old_controls[i].type, 
...
{			  
	...
	// restore original settings.
		for (i = 0; i < N_KEY_CFG_FN; i++)
		{
			Controller->set_controller_function(Cfg_key_elements[i].fn_id, m_old_controls[i].type, 
									m_old_controls[i].value, m_old_controls[i].flags);
		}
		break;
	}
...
void joy_cfg_screen::process(int res)
{
	...
	// restore original settings.
		for (i = 0; i < N_KEY_CFG_FN; i++)
		{
			Controller->set_controller_function(Cfg_joy_elements[i].fn_id, m_old_controls[i].type, 
									m_old_controls[i].value, m_old_controls[i].flags);
		}
		break;
	}
...
// determine which configuration pipe to go down
	...
	switch (ctype_fn[0]) 
	{
	case ctKey:	strcpy(str, TXT_CFGHELP_KEYB); break;
	case ctAxis: 
	case ctMouseAxis: strcpy(str, TXT_CFGHELP_JOY); break;
	case ctButton:
	case ctMouseButton:
	case ctPOV: 
	case ctPOV2:
	case ctPOV3:
	case ctPOV4: strcpy(str, TXT_CFGHELP_BTNS); break;
	default:
		str[0] = 0;
	}
// open a small window
...
// performs custom gadget deinitialization and reinitilization
	...
	{
	case IDV_KCONFIG: cfgdata->curcfg = cfgdata->keycfg; break;
	case IDV_CCONFIG: cfgdata->curcfg = cfgdata->joycfg; break;
...
// open menu
	...
	menu.SetCurrentOption((mode==CTLCONFIG_WPNSEL) ? IDV_WPNSEL : (mode==CTLCONFIG_CONTROLLER) ? IDV_CCONFIG : IDV_KCONFIG);
...
//		}
		...
		{
		case IDV_KCONFIG: 		keycfg.process(res);	break;
		case IDV_CCONFIG:		joycfg.process(res);	break;
		case IDV_WPNSEL:		wpncfg.process(res);	break;
		};
	}
	while (1);

...
#define UID_LOOPING			200
	...
	UITextItem wndItem = UITextItem(TXT_DEMO_PLAY_OPTIONS);
	wndtitle.Create(&window, &wndItem, 0, cury, UIF_CENTER| UIF_FIT );cury+=30;
	if(Demo_play_fast)
	{
		UITextItem fpsItem = UITextItem(sztemp);
		sprintf(sztemp,TXT_MINFPS,(float)1/Max_frametime);
		fps_min.Create(&window, &fpsItem, 0, cury, UIF_CENTER| UIF_FIT );cury+=20;
		sprintf(sztemp,TXT_MAXFPS,(float)1/Min_frametime);
		fps_max.Create(&window, &fpsItem, 0, cury, UIF_CENTER| UIF_FIT );cury+=20;
		sprintf(sztemp,TXT_AVGFSP,(float)1/(Avg_frametime/Frames_counted));
		fps_avg.Create(&window, &fpsItem, 0, cury, UIF_CENTER| UIF_FIT );cury+=20;
	}
	//cury+=20;
	UITextItem btnItem = UITextItem(TXT_PLAYLOOPING);
	loop_btn.Create(&window, UID_LOOPING, &btnItem, 0, cury, BTN_WIDTH*1.5, 32,UIF_CENTER| UIF_FIT); cury+=35;
	UITextItem okBtnItem = UITextItem(TXT_DONE);
	ok_btn.Create(&window, UID_OK, &okBtnItem, 0, DEMO_MENU_HEIGHT-70, BTN_WIDTH, 32,UIF_CENTER| UIF_FIT); 

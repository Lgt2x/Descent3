...
bool ConfigItem::Create(NewUIGameWindow *parentwnd,int type,int flags,int x,int y,int w,int h,char *label,int item_x)
{
	if(m_bAlive)
		return false;
	if(!parentwnd)
...
bool ConfigItem::Create(NewUIGameWindow *parentwnd,int type,char *label,int x,int y,int w,int h,int initialvalue,int flags,int item_x)
{
...
bool ConfigItem::Create(NewUIGameWindow *parentwnd,int type,char *label,int x,int y,int w,int h,bool initialvalue,int flags,int item_x)
{
	if(!Create(parentwnd,type,flags,x,y,w,h,label,item_x))
		return false;
...
bool ConfigItem::Create(NewUIGameWindow *parentwnd,int type,char *label,int x,int y,int w,int h,float initialvalue,int flags,int item_x)
{
	if(!Create(parentwnd,type,flags,x,y,w,h,label,item_x))
		return false;
...
void ConfigItem::Add(int count, ... )
{
	...
	case CIT_HOTSPOTLIST:
		...
		for(i=0;i<m_iNumIDs;i++){
			...
			ASSERT(m_rbList[i]);
			if(i)
				m_rbList[i]->Create(m_hWnd,m_rbList[i-1],m_iID[i],m_tiList[i],m_X+curr_x,m_Y+curr_y,m_tiList[i]->width(),m_tiList[i]->height(),UIF_FIT|UIRB_NOBUTTON);
			else
				m_rbList[i]->Create(m_hWnd,NULL,m_iID[i],m_tiList[i],m_X+curr_x,m_Y+curr_y,m_tiList[i]->width(),m_tiList[i]->height(),UIF_FIT|UIRB_NOBUTTON);
	...
	case CIT_ONOFFBUTTON:{
		...
		//make sure allocation ok
		ASSERT(m_bList[0]);
		m_bList[0]->Create(m_hWnd,m_iID[0],m_tiList[0],m_X+curr_x,m_Y+curr_y,m_tiList[0]->width(),m_tiList[0]->height(),UIF_FIT);
		//Initialize Config Item to initial value
	...
	case CIT_CHECKBOX:
	{
		...
		//make sure allocation ok
		ASSERT(m_hsList[0]);
		m_hsList[0]->Create(m_hWnd,m_iID[0],-1,m_tiList[0],m_tiList[1],m_X+curr_x,m_Y+curr_y,m_tiList[0]->width(),m_tiList[0]->height(),UIF_FIT);
		//Initialize Config Item to initial value
		ASSERT(m_iValueType==CIV_BOOL);
		if(m_bInitial)
	...
	case CIT_RADIOBUTTON:
	{
		...
		for(i=0;i<count;i++){
			...
			ASSERT(m_rbList[i]);
			if(i)
				m_rbList[i]->Create(m_hWnd,m_rbList[i-1],m_iID[i],m_tiList[i],m_X+curr_x+offset,m_Y+curr_y,m_W,m_H,UIF_FIT);
			else
				m_rbList[i]->Create(m_hWnd,NULL,m_iID[i],m_tiList[i],m_X+curr_x+offset,m_Y+curr_y,m_W,m_H,UIF_FIT);

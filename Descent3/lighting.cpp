...
void ApplyLightingToExternalRoom (vector *pos,int roomnum,float light_dist,float red_scale,float green_scale,float blue_scale,vector *light_direction,float dot_range)
{
	int i,lm_handle,t;
	ushort *dest_data;
	vector rad;
	...
	for (i=0;i<rp->num_faces;i++)
	{
		...
		if(Light_max_xyz.y           < fp->min_xyz.y  ||
      fp->max_xyz.y < Light_min_xyz.y            ||
		Light_max_xyz.x           < fp->min_xyz.x  ||
      fp->max_xyz.x < Light_min_xyz.x            ||
...
void ApplyLightingToSubmodel (object *obj,poly_model *pm,bsp_info *sm,float light_dist,float red_scale,float green_scale,float blue_scale,float dot_range)
{
	matrix mat;
	vector Light_min_xyz;
	vector Light_max_xyz;
	...
	{
		// check to see if this face is within reach
		if(Light_max_xyz.y           < sm->face_min[i].y  ||
      sm->face_max[i].y < Light_min_xyz.y            ||
		Light_max_xyz.x           < sm->face_min[i].x  ||
      sm->face_max[i].x < Light_min_xyz.x            ||
      Light_max_xyz.z           < sm->face_min[i].z  ||
      sm->face_max[i].z < Light_min_xyz.z) continue;
	...
	for (i=0;i<sm->num_children;i++)
		ApplyLightingToSubmodel(obj,pm,&pm->submodel[sm->children[i]], light_dist,red_scale,green_scale,blue_scale,dot_range);
...
void ApplySpecularLightingToWall (room *rp,face *fp,vector *pos,float light_dist,float dist_from_plane,float red_scale,float green_scale,float blue_scale)
{
...
void ApplyVolumeLightToObject (vector *pos,object *obj,float light_dist,float red_scale,float green_scale,float blue_scale,vector *light_direction,float dot_range)
{
	...
	if ((light_dist-mag)>obj->effect_info->spec_mag && Detail_settings.Specular_lighting && !(Object_info[obj->id].lighting_info.flags & OLF_NO_SPECULARITY))
	{
...
void ApplyLightingToObjects(vector *pos,int roomnum,float light_dist,float red_scale,float green_scale,float blue_scale,vector *light_direction,float dot_range)
{
	...
	for (i=0;i<num_objects;i++)
	{
		...
		{
			ApplyLightingToExternalRoom (pos,obj->id,light_dist,red_scale,green_scale,blue_scale,light_direction,dot_range);
			continue;
		}
		...
		{
			if (obj->effect_info && ((obj->effect_info->type_flags & EF_VOLUME_LIT) || obj->lighting_render_type==LRT_GOURAUD || obj->type==OBJ_POWERUP) )
			{
...
void ApplyLightingToTerrain (vector *pos,int cellnum,float light_dist,float red_scale,float green_scale,float blue_scale,vector *light_direction,float dot_range)
{
	...
	ApplyLightingToObjects (pos,MAKE_ROOMNUM(cellnum),light_dist,red_scale,green_scale,blue_scale,light_direction,dot_range);
	...
	for (int y=0;y<yres;y++,base_vector-=(facematrix.uvec*yspacing),texel_num+=xres)
	{
		...
		for (int x=0;x<xres;x++,element_vec+=(facematrix.rvec*xspacing))
		{
			...
			for (int i=0;i<num;i++)
			{
				...
				vector incident_norm=element_vec-SpecialFaces[fp->special_handle].spec_instance[i].bright_center;

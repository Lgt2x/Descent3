...
void DoMultiAllowed(void)
{
	...
	add_ship_hs.Create(&menu_wnd,10,0,&add_off,&add_on,10,(yrow1+(list_ysize/2))-25,130, 30,UIF_FIT|UIF_CENTER);
	remove_ship_hs.Create(&menu_wnd,11,0,&remove_off,&remove_on,10,(yrow1+(list_ysize/2))-5,130, 30,UIF_FIT|UIF_CENTER);

	add_gen_hs.Create(&menu_wnd,12,0,&add_off,&add_on,10,(yrow2+(list_ysize/2))-25,130, 30,UIF_FIT|UIF_CENTER);
	remove_gen_hs.Create(&menu_wnd,13,0,&remove_off,&remove_on,10,(yrow2+(list_ysize/2))-5,130, 30,UIF_FIT|UIF_CENTER);
	...
	// Exit out Hotspot
	exit_hs.Create(&menu_wnd,UID_OK,KEY_ESC,&exit_off_text,&exit_on_text, 10,420, 180, 30,UIF_FIT|UIF_CENTER); cury+=20; 
	cancel_hs.Create(&menu_wnd,UID_CANCEL,0,&cancel_off_text,&cancel_on_text, 10,445, 180, 30,UIF_FIT|UIF_CENTER); cury+=20; 
		...
		case UID_OK:
			{
				...
				if (i == MAX_SHIPS) {
					DoMessageBox(TXT_MENUMULTIPLAYER,"You must have at least one allowed ship.",MSGBOX_OK,UICOL_WINDOW_TITLE,UICOL_TEXT_NORMAL);
					break;
				}
...
#define MULTI_OPT_HOTSPOT_Y		360
	...
	time_limit_edit.Create(&main_wnd,id++,MULTI_OPT_EDITS_X_COL1,cury,100,15,0);cury+=20;  
	cury+=20;
	...
	kill_goal_edit.Create(&main_wnd,id++,MULTI_OPT_EDITS_X_COL1,cury,100,15,0); cury+=20;
	cury+=20;
	...
	if(alloptions)
	{
		...
		max_players_edit.Create(&main_wnd,id++,MULTI_OPT_EDITS_X_COL1,cury,100,15,0); cury+=20;
	}
	cury+=20;
	...
	NewUIEdit pps_edit;
	pps_edit.Create(&main_wnd,id++,MULTI_OPT_EDITS_X_COL2,cury,100,15,0); cury+=20;
	cury+=20;
	...
	respawn_edit.Create(&main_wnd,id++,MULTI_OPT_EDITS_X_COL2,cury,100,15,0); cury+=20;
	...
	if(alloptions)
	{
		...
		{
			cs_mode_hs.Create(&main_wnd,CS_MODE_HS_ID,KEY_C,&cs_mode_unsel_text_off,&cs_mode_unsel_text_on,MULTI_OPT_TOGGLES_X,cury,180,30,UIF_FIT); 
			cs_y = cury;
			cury+=20;

			pp_mode_hs.Create(&main_wnd,PP_MODE_HS_ID,KEY_C,&pp_mode_sel_text_off,&pp_mode_sel_text_on,MULTI_OPT_TOGGLES_X,cury,180,30,UIF_FIT);
			pp_y = cury;
			cury+=20;

			pcs_mode_hs.Create(&main_wnd,PS_MODE_HS_ID,KEY_C,&pcs_mode_unsel_text_off,&pcs_mode_unsel_text_on,MULTI_OPT_TOGGLES_X,cury,180,30,UIF_FIT);
			pcs_y = cury;
			cury+=20;
		}
		else if (Netgame.flags & NF_PERMISSABLE)
		{
			...
			pp_mode_hs.Create(&main_wnd,PP_MODE_HS_ID,KEY_C,&pp_mode_unsel_text_off,&pp_mode_unsel_text_on,MULTI_OPT_TOGGLES_X,cury,180,30,UIF_FIT);
			pp_y = cury;
			cury+=20;

			pcs_mode_hs.Create(&main_wnd,PS_MODE_HS_ID,KEY_C,&pcs_mode_sel_text_off,&pcs_mode_sel_text_on,MULTI_OPT_TOGGLES_X,cury,180,30,UIF_FIT);
			pcs_y = cury;
			cury+=20;
		}
		else
		{
			cs_mode_hs.Create(&main_wnd,CS_MODE_HS_ID,KEY_C,&cs_mode_sel_text_off,&cs_mode_sel_text_on,MULTI_OPT_TOGGLES_X,cury,180,30,UIF_FIT); 
			cs_y = cury;
			cury+=20;
			pp_mode_hs.Create(&main_wnd,PP_MODE_HS_ID,KEY_C,&pp_mode_unsel_text_off,&pp_mode_unsel_text_on,MULTI_OPT_TOGGLES_X,cury,180,30,UIF_FIT);
			pp_y = cury;
		...
		{
			rot_vel_hs.Create(&main_wnd,ROT_VEL_HS_ID,0,&rot_vel_sel_txt_off,&rot_vel_sel_txt_on,MULTI_OPT_TOGGLES_COL2_X,cury,180,30,UIF_FIT); 
		}
		else
		{
			rot_vel_hs.Create(&main_wnd,ROT_VEL_HS_ID,0,&rot_vel_unsel_txt_off,&rot_vel_unsel_txt_on,MULTI_OPT_TOGGLES_COL2_X,cury,180,30,UIF_FIT); 
		}
		rotvely = cury;
		cury+=20;
	}
	...
	{
		acc_weap_hs.Create(&main_wnd,ACC_WEAP_HS_ID,KEY_A,&acc_weap_coll_sel_off,&acc_weap_coll_sel_on,MULTI_OPT_TOGGLES_COL2_X,cury,180,30,UIF_FIT); 
	}
	else
	{
		acc_weap_hs.Create(&main_wnd,ACC_WEAP_HS_ID,KEY_A,&acc_weap_coll_unsel_off,&acc_weap_coll_unsel_on,MULTI_OPT_TOGGLES_COL2_X,cury,180,30,UIF_FIT); 
	}
	acc_weap_y = cury;
	cury+=20;
	...
	{
		bright_players_hs.Create(&main_wnd,BRIGHT_SHIP_HS_ID,KEY_B,&bright_players_sel_off,&bright_players_sel_on,MULTI_OPT_TOGGLES_COL2_X,cury,180,30,UIF_FIT); 
	}
	else
	{
		bright_players_hs.Create(&main_wnd,BRIGHT_SHIP_HS_ID,KEY_B,&bright_players_unsel_off,&bright_players_unsel_on,MULTI_OPT_TOGGLES_COL2_X,cury,180,30,UIF_FIT); 
	}
	...
	{
		powerup_hs.Create(&main_wnd,RAND_POWERUP_ID,KEY_B,&powerup_respawn_sel_off,&powerup_respawn_sel_on,MULTI_OPT_TOGGLES_COL2_X,cury,180,30,UIF_FIT); 
	}
	else
	{
		powerup_hs.Create(&main_wnd,RAND_POWERUP_ID,KEY_B,&powerup_respawn_unsel_off,&powerup_respawn_unsel_on,MULTI_OPT_TOGGLES_COL2_X,cury,180,30,UIF_FIT); 
	}
	powerup_y = cury;
	cury+=20;
	...
	{
		mlook_hs.Create(&main_wnd,MLOOK_HS_ID,KEY_B,&mouselookers_sel_off,&mouselookers_sel_on,MULTI_OPT_TOGGLES_COL2_X,cury,180,30,UIF_FIT); 
	}
	else
	{
		mlook_hs.Create(&main_wnd,MLOOK_HS_ID,KEY_B,&mouselookers_unsel_off,&mouselookers_unsel_on,MULTI_OPT_TOGGLES_COL2_X,cury,180,30,UIF_FIT); 
	}
	mlook_y = cury;
	cury+=20;
	...
	UIHotspot ship_hs;
	ship_hs.Create(&main_wnd,SHIP_ALLOW_HS_ID,KEY_S,&ship_off_text,&ship_on_text,0,cury,180,30,UIF_FIT|UIF_CENTER); cury+=20;
	return_id=UID_OK;
	cury+=10;
	//hsmid = MULTI_OPT_HOTSPOT_X_MID - (DLLgrtext_GetTextLineWidth(TXT_GEN_PREVMENU)/2);
	UIHotspot exit_hs;
	exit_hs.Create(&main_wnd,UID_OK,KEY_ESC,&exit_off_text,&exit_on_text, 0,cury, 180, 30,UIF_FIT|UIF_CENTER); cury+=20;
	cancel_id = UID_CANCEL;
///	hsmid = MULTI_OPT_HOTSPOT_X_MID - (DLLgrtext_GetTextLineWidth(TXT_GEN_CANCEL)/2);
	UIHotspot cancel_hs;
	cancel_hs.Create(&main_wnd,UID_CANCEL,0,&cancel_off_text,&cancel_on_text, 0,cury, 180, 30,UIF_FIT|UIF_CENTER); cury+=20;
	// Bash some values
	...
	if(alloptions)
	{
			if(res==CS_MODE_HS_ID)
			{
				...
				cs_mode_hs.Create(&main_wnd,CS_MODE_HS_ID,KEY_C,&cs_mode_sel_text_off,&cs_mode_sel_text_on,MULTI_OPT_TOGGLES_X,cs_y,180,30,UIF_FIT); 
				pp_mode_hs.Create(&main_wnd,PP_MODE_HS_ID,KEY_C,&pp_mode_unsel_text_off,&pp_mode_unsel_text_on,MULTI_OPT_TOGGLES_X,pp_y,180,30,UIF_FIT);
				pcs_mode_hs.Create(&main_wnd,PS_MODE_HS_ID,KEY_C,&pcs_mode_unsel_text_off,&pcs_mode_unsel_text_on,MULTI_OPT_TOGGLES_X,pcs_y,180,30,UIF_FIT);
			...
			else if(res==PP_MODE_HS_ID)
			{
				...
				cs_mode_hs.Create(&main_wnd,CS_MODE_HS_ID,KEY_C,&cs_mode_unsel_text_off,&cs_mode_unsel_text_on,MULTI_OPT_TOGGLES_X,cs_y,180,30,UIF_FIT); 
				pp_mode_hs.Create(&main_wnd,PP_MODE_HS_ID,KEY_C,&pp_mode_sel_text_off,&pp_mode_sel_text_on,MULTI_OPT_TOGGLES_X,pp_y,180,30,UIF_FIT);
				pcs_mode_hs.Create(&main_wnd,PS_MODE_HS_ID,KEY_C,&pcs_mode_unsel_text_off,&pcs_mode_unsel_text_on,MULTI_OPT_TOGGLES_X,pcs_y,180,30,UIF_FIT);
				Netgame.flags |= NF_PEER_PEER;
				Netgame.flags &= ~NF_PERMISSABLE;
			...
			else if (res==PS_MODE_HS_ID)
			{
				...
				cs_mode_hs.Create(&main_wnd,CS_MODE_HS_ID,KEY_C,&cs_mode_unsel_text_off,&cs_mode_unsel_text_on,MULTI_OPT_TOGGLES_X,cs_y,180,30,UIF_FIT); 
				pp_mode_hs.Create(&main_wnd,PP_MODE_HS_ID,KEY_C,&pp_mode_unsel_text_off,&pp_mode_unsel_text_on,MULTI_OPT_TOGGLES_X,pp_y,180,30,UIF_FIT);
				pcs_mode_hs.Create(&main_wnd,PS_MODE_HS_ID,KEY_C,&pcs_mode_sel_text_off,&pcs_mode_sel_text_on,MULTI_OPT_TOGGLES_X,pcs_y,180,30,UIF_FIT);
				Netgame.flags |= NF_PERMISSABLE;
				Netgame.flags &= ~NF_PEER_PEER;
			...
			else if(res==ROT_VEL_HS_ID)
			{
				...
				if(Netgame.flags & NF_SENDROTVEL)
				{
					rot_vel_hs.Create(&main_wnd,ROT_VEL_HS_ID,0,&rot_vel_unsel_txt_off,&rot_vel_unsel_txt_on,MULTI_OPT_TOGGLES_COL2_X,rotvely,180,30,UIF_FIT); 
					Netgame.flags &= ~NF_SENDROTVEL;
				}
				else
				{
					rot_vel_hs.Create(&main_wnd,ROT_VEL_HS_ID,0,&rot_vel_sel_txt_off,&rot_vel_sel_txt_on,MULTI_OPT_TOGGLES_COL2_X,rotvely,180,30,UIF_FIT); 
					Netgame.flags |= NF_SENDROTVEL;
				}
			}
		...
		else if (res==ACC_WEAP_HS_ID)
		{
			...
			if(Netgame.flags & NF_USE_ACC_WEAP)
			{
				acc_weap_hs.Create(&main_wnd,ACC_WEAP_HS_ID,KEY_A,&acc_weap_coll_unsel_off,&acc_weap_coll_unsel_on,MULTI_OPT_TOGGLES_COL2_X,acc_weap_y,180,30,UIF_FIT); 
				Netgame.flags &= ~NF_USE_ACC_WEAP;
			}
			else
			{
				acc_weap_hs.Create(&main_wnd,ACC_WEAP_HS_ID,KEY_A,&acc_weap_coll_sel_off,&acc_weap_coll_sel_on,MULTI_OPT_TOGGLES_COL2_X,acc_weap_y,180,30,UIF_FIT); 
				Netgame.flags |= NF_USE_ACC_WEAP;
		...
		else if (res==BRIGHT_SHIP_HS_ID)
		{
			...
			if(Netgame.flags & NF_BRIGHT_PLAYERS)
			{
				bright_players_hs.Create(&main_wnd,BRIGHT_SHIP_HS_ID,KEY_B,&bright_players_unsel_off,&bright_players_unsel_on,MULTI_OPT_TOGGLES_COL2_X,bright_players_y,180,30,UIF_FIT); 
				Netgame.flags &= ~NF_BRIGHT_PLAYERS;
			}
			else
			{
				bright_players_hs.Create(&main_wnd,BRIGHT_SHIP_HS_ID,KEY_B,&bright_players_sel_off,&bright_players_sel_on,MULTI_OPT_TOGGLES_COL2_X,bright_players_y,180,30,UIF_FIT); 
				Netgame.flags |= NF_BRIGHT_PLAYERS;
			}
		}
		...
		else if (res==RAND_POWERUP_ID)
		{
			...
			if(Netgame.flags & NF_RANDOMIZE_RESPAWN)
			{
				powerup_hs.Create(&main_wnd,RAND_POWERUP_ID,KEY_B,&powerup_respawn_unsel_off,&powerup_respawn_unsel_on,MULTI_OPT_TOGGLES_COL2_X,powerup_y,180,30,UIF_FIT); 
				Netgame.flags &= ~NF_RANDOMIZE_RESPAWN;
			}
			else
			{
				powerup_hs.Create(&main_wnd,RAND_POWERUP_ID,KEY_B,&powerup_respawn_sel_off,&powerup_respawn_sel_on,MULTI_OPT_TOGGLES_COL2_X,powerup_y,180,30,UIF_FIT); 
				Netgame.flags |= NF_RANDOMIZE_RESPAWN;
			}
		}
		...
		else if (res==MLOOK_HS_ID)
		{
			...
			if(Netgame.flags & NF_ALLOW_MLOOK)
			{
				mlook_hs.Create(&main_wnd,MLOOK_HS_ID,KEY_B,&mouselookers_unsel_off,&mouselookers_unsel_on,MULTI_OPT_TOGGLES_COL2_X,mlook_y,180,30,UIF_FIT); 
				Netgame.flags &= ~NF_ALLOW_MLOOK;
			}
			else
			{
				mlook_hs.Create(&main_wnd,MLOOK_HS_ID,KEY_B,&mouselookers_sel_off,&mouselookers_sel_on,MULTI_OPT_TOGGLES_COL2_X,mlook_y,180,30,UIF_FIT); 
				Netgame.flags |= NF_ALLOW_MLOOK;
			}
		}
...
void ShowNetgameInfo(network_game *game)
{
	...
	sheet->NewGroup(game->name,GAME_INFO_COL1,cury); cury+=15;

...
object *CreateSubobjectDebrisDirected(object *parent, int subobj_num,vector *dir,float explosion_mag,int death_flags)
{
	...
	if(obj->mtype.phys_info.velocity.x > 100000.0f) obj->mtype.phys_info.velocity.x = 100000.0f;
	if(obj->mtype.phys_info.velocity.y > 100000.0f) obj->mtype.phys_info.velocity.y = 100000.0f;
	if(obj->mtype.phys_info.velocity.z > 100000.0f) obj->mtype.phys_info.velocity.z = 100000.0f;
	if(obj->mtype.phys_info.velocity.x < -100000.0f) obj->mtype.phys_info.velocity.x = -100000.0f;
	if(obj->mtype.phys_info.velocity.y < -100000.0f) obj->mtype.phys_info.velocity.y = -100000.0f;
	if(obj->mtype.phys_info.velocity.z < -100000.0f) obj->mtype.phys_info.velocity.z = -100000.0f;
...
void CreateSplintersFromBody (object *obj,float explosion_mag,float lifetime)
{
	...
	for (i=0;i<num_splinters;i++)
	{
		...
		vector dest,center;
		int num_verts=std::min(static_cast<int>(sm->faces[facenum].nverts),MAX_VERTS_PER_SPLINTER);
		...
		else
		{
			...
			splinter_obj->mtype.phys_info.velocity = (obj->mtype.phys_info.velocity)+(((subvec*obj->size)/lifetime)*4);
...
char *dead_object_types[] = { "swatter", "swatter(deadmodel)",
										"Hangturret", "Securityturret(DEAD)",
										"Lance", "Lance(DEAD)",
										"L10 swatter", "swatter(deadmodel)",
									 };
...
void DestroyObject(object *objp,float explosion_mag,int death_flags)
{
	...
	if (death_flags & DF_FIREBALL) {
		...
		switch (DEATH_EXPL_SIZE(death_flags)) {
			case DF_EXPL_SMALL:	size_scale = 0.5f; break;
			case DF_EXPL_MEDIUM:	size_scale = 1.0f; break;
			case DF_EXPL_LARGE:	size_scale = 1.6f; break;
			default: Int3();
		}
		//Create big fireball
	...
	if (death_flags & DF_REMAINS) 
	{		//Make object do nothing
...
void DoDyingFrame(object *objp)
{
	...
	if (death_flags & DF_DELAY_SPARKS) {
		...
		dying_norm=std::min(dying_norm,1.0f);
		//Do deform stuff
	...
	if (death_flags & DF_DELAY_FIREBALL) {
		...
		if ((Gametime-objp->ctype.dying_info.last_fireball_time>.01) && (ps_rand()%3)==0)
		{
			...
			GetPolyModelPointInWorld (&dest,&Poly_models[objp->rtype.pobj_info.model_num], &objp->pos, &objp->orient, 0, &sm->verts[vertnum] );
...
void DoGravityFieldEffect (object *obj)
{
	...
	for (i=0; i<=Highest_object_index; i++ )	
	{
		...
		if (!(hit_obj_ptr->type==OBJ_PLAYER || hit_obj_ptr->type==OBJ_ROBOT || hit_obj_ptr->type==OBJ_WEAPON || hit_obj_ptr->type==OBJ_POWERUP || hit_obj_ptr->type == OBJ_CLUTTER || (hit_obj_ptr->type == OBJ_BUILDING && hit_obj_ptr->ai_info)))
			continue;
...
void DoExplosionFrame(object *obj)
{
	...
	{	
		if (obj->id==GRAVITY_FIELD_INDEX)
		{
			...
			if (OBJECT_OUTSIDE (obj))
				CreateBlastRing (&obj->pos,BLAST_RING_INDEX,DAMAGE_RING_TIME,obj->ctype.blast_info.max_size*3,obj->roomnum,1);
			else
...
//object *object_create_explosion_sub(object *explode_obj_ptr, short segnum, vms_vector * position, fix size, int vclip_type, fix maxdamage, fix maxdistance, fix maxforce, int parent )
void DoConcussiveForce(object *explode_obj_ptr, int parent_handle,float player_scalar)
{
	...
	{
		effect_distance = explode_obj_ptr->impact_size * ((Gametime - explode_obj_ptr->creation_time) / explode_obj_ptr->impact_time);
...
//			if ( (hit_obj_ptr!=explode_obj_ptr) && !(hit_obj_ptr->flags&OF_DEAD) && ((hit_obj_ptr->type==OBJ_WEAPON /*&& (hit_obj_ptr->id==PROXIMITY_ID || hit_obj_ptr->id==SUPERPROX_ID || hit_obj_ptr->id==PMINE_ID)*/) || (hit_obj_ptr->type == OBJ_CNTRLCEN) || (hit_obj_ptr->type==OBJ_PLAYER) || ((hit_obj_ptr->type==OBJ_ROBOT) && ((Objects[parent].type != OBJ_ROBOT) || (Objects[parent].id != hit_obj_ptr->id))))) {
			hit_obj_ptr = &Objects[i];
			//Check for types that get concussive force
			if (!((hit_obj_ptr->type == OBJ_WEAPON) ||
					(hit_obj_ptr->type == OBJ_ROBOT) ||
					(hit_obj_ptr->type == OBJ_BUILDING) || 
					(hit_obj_ptr->type == OBJ_PLAYER) ||
					(hit_obj_ptr->type == OBJ_DOOR) ||
					(hit_obj_ptr->type == OBJ_CLUTTER)))
				continue;
			if((hit_obj_ptr!=explode_obj_ptr) && !(hit_obj_ptr->flags&OF_DEAD)) 
			{
				...
				if(hit_obj_ptr->movement_type == MT_PHYSICS || hit_obj_ptr->movement_type == MT_WALKING)
				{
					if((hit_obj_ptr->mtype.phys_info.flags & PF_IGNORE_OWN_CONC_FORCES) && ObjGetUltimateParent(hit_obj_ptr) == ObjGetUltimateParent(explode_obj_ptr))
						continue;
				}
				// The distance is actually the objects' centers minus some of the hit object's radius (I set it to 80%)
				if((hit_obj_ptr->flags & OF_POLYGON_OBJECT) &&
					 hit_obj_ptr->type != OBJ_WEAPON &&
		          hit_obj_ptr->type != OBJ_POWERUP &&
		          hit_obj_ptr->type != OBJ_DEBRIS)
				{
					vector pos;
					pos = hit_obj_ptr->pos + hit_obj_ptr->wall_sphere_offset;
					dist = vm_VectorDistanceQuick( &pos, &explode_obj_ptr->pos ) - Poly_models[hit_obj_ptr->rtype.pobj_info.model_num].wall_size;
				}
				else
				{
...
void CreateBlueBlastRing(vector *pos,int index,float lifetime,float max_size,int roomnum,int objnum,int force_up)
{
	if (! Viewer_object)
		return;
...
void DrawColoredDisk (vector *pos,float r,float g,float b,float inner_alpha,float outer_alpha,float size,ubyte saturate,ubyte lod)
{
	rend_SetZBufferWriteMask (0);
...
void DrawColoredRing(vector *pos,float r,float g,float b,float inner_alpha,float outer_alpha,float size,float inner_ring_ratio,ubyte saturate,ubyte lod)
{

	...
	for(i=0;i<TG_NumLines;i++)
	{
		...
		if(TG_Lines[goal_index].is_active){
			...
			if(TG_CurrObjectiveArrow>BAD_BITMAP_HANDLE)
			{
				...
				rend_DrawScaledBitmap(x-small_height-2,wrap_y,x-2,wrap_y+small_height,TG_CurrObjectiveArrow,0,0,1,1);
...
#define TOOLTIP_SPEED	400.0f
	...
	if(TG_MouseOffset_x==-1)
	{
		...
		while( index>0 && (!(data[index]&OPAQUE_FLAG)) ) index--;
	...
	if(mouse_x+TG_TT_curr.w>Telcom_system.Monitor_coords[MONITOR_MAIN].right||mouse_x+TG_TT_dest.w>Telcom_system.Monitor_coords[MONITOR_MAIN].right)
	{
		//we must adjust
		mouse_x -= TG_TT_curr.w;
	}

	rend_DrawScaledChunkedBitmap(&TG_Tooltipbitmap,mouse_x+TG_MouseOffset_x,mouse_y+TG_MouseOffset_y,TG_TT_curr.w,TG_TT_curr.h,255);
	rend_SetFiltering (1);

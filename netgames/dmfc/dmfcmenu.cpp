...
#define FLASHRATE	700.0	//alphas per second
	...
	while(units>0){
		if(m_AlphaDir){
			...
			ubyte amount = std::min(255 - a,units);
			units -= amount;
			a += amount;
			if(a>=255){
				a = 255;
			...
			//find the amount to adjust
			ubyte amount = std::min(static_cast<int>(a),units);
			units -= amount;
			a -= amount;
			if(a<=0){
				a = 0;
	...
	m_bAtBottom = (m_bMoreToScroll)?((CurrSubMenu<end_index-2)?false:true):((CurrSubMenu<end_index)?false:true);
	...
	for(int i=m_iTopIndex;i<end_index;i++){
		...
		if(draw_text){
			...
			case MIT_STATE:
				{
					if(i==m_iState) strcpy(buff,checked); else strcpy(buff,"  ");
...
bool MenuItem::HasSubMenus(void)
{
...
{
	return basethis->GetNumBannedPlayers();
}
			...
			for(j=0;j<MAX_TEAMS;j++){
				if(ShouldHave[j]<TeamCount[j]){
					...
					for(pn=0;pn<DLLMAX_PLAYERS;pn++){
						if( (basethis->CheckPlayerNum(pn)) && (basethis->Players[pn].team==j) && (basethis->Players[pn].team!=-1)){
...
{
	basethis->SwitchShowHudCallsignLevel(i);
}
...
{
	basethis->CallOnSaveStatsToFile();
}

...
{	
	...
	if(m_highlight_bmp==-1) m_highlight_bmp = BAD_BITMAP_HANDLE;
	if(m_background_bmp==-1) m_background_bmp = BAD_BITMAP_HANDLE;
...
bool CDmfcStats::CanScrollDown()
{
	return (m_selected_index<(m_MaxCount-1))?true:false;
}

// returns true if the player list can scroll up any more
bool CDmfcStats::CanScrollUp()
{
	return (bool)(m_selected_index>0);
}
...
void CDmfcStats::Render()
{
	...
	DLLRenderHUDQuad(PLIST_WIN_X,PLIST_WIN_Y,PLIST_WIN_W,PLIST_WIN_H,0,0,1,1,m_background_bmp,perc_done*200.0f,0);

	DLLgrtext_SetFont(basethis->Game_fonts[HUD_FONT_INDEX]);
	int l_width = DLLRenderHUDGetTextLineWidth(m_GameName);
	DLLRenderHUDText(GR_WHITE,m_alpha,0,(PLIST_WIN_W/2)-(l_width/2),curr_y,m_GameName); curr_y += (m_font_height+ROW_PIXEL_GAP);
...
redisplay_list:
	...
	for(i=m_top_index;i<m_MaxCount;i++){
		...
		if(m_selected_index==i){
			DLLRenderHUDQuad(m_x_start_plist,curr_y,PLIST_WIN_W,m_font_height,0,0,1,1,m_highlight_bmp,perc_done * 128.0f,0);
		}		

		need_to_display_observer_icon = (pr->state==STATE_INGAME)?(basethis->IsPlayerObserver(pr->pnum)?true:false):false;
			...
			if(m_PLColInfo[c].type!=DSCOL_BMP){
				...
				if(need_to_display_observer_icon){
					if(m_flags&DSIF_SHOW_OBSERVERICON && m_PLColInfo[c].type==DSCOL_PILOT_NAME){
						...
						DLLRenderHUDQuad(curr_x,curr_y,m_font_height,m_font_height,0,0,1,1,basethis->hBitmapObserver,m_alpha,0);
				...
				case DSCOLOR_NORMAL:
					...
					break;
				case DSCOLOR_TEAM:
					color = (m_PLColInfo[c].type==DSCOL_PILOT_NAME)?(basethis->GetTeamColor(pr->team)):((pr->state==STATE_INGAME)?(basethis->GetTeamColor(pr->team)):(GR_GREY));
					break;
				...
				(*m_PLColCallbackBMP)(m_translate[i],c,curr_x,curr_y,m_PLColInfo[c].width,m_font_height+ROW_PIXEL_GAP,m_alpha);
			}
...
skip_line:
	...
	if((m_flags&DSIF_SEPERATE_BY_TEAM) && (teams_displayed<DLLMAX_TEAMS) && ((curr_y+m_font_height+ROW_PIXEL_GAP) < PLIST_WIN_H+PLIST_WIN_Y ) ){
	...
	DLLRenderHUDQuad(DLIST_WIN_X,DLIST_WIN_Y,DLIST_WIN_W,DLIST_WIN_H,0,0,1,1,m_background_bmp,perc_done*200.0f,0);
	...
	DLLRenderHUDText(GR_WHITE,m_alpha,0,DLIST_WIN_X+(DLIST_WIN_W/2)-(l_width/2),curr_y,buffer); curr_y += (m_font_height+ROW_PIXEL_GAP);
	...
	if(m_flags&DSIF_SHOW_PIC && pr->state==STATE_INGAME && !is_dedicated_server){
		...
		if(basethis->PilotPicBmpHandles[pr->pnum]>BAD_BITMAP_HANDLE){
			...
			DLLRenderHUDQuad(DLIST_WIN_X+(DLIST_WIN_W/2)-32,curr_y,64,64,0,0,1,1,basethis->PilotPicBmpHandles[pr->pnum],m_alpha,0);
			curr_y += 70;
	...
	l_width = DLLRenderHUDGetTextLineWidth(buffer);
	max_w = l_width;
	DLLRenderHUDText(GR_WHITE,m_alpha,0,x_to_use,curr_y,buffer); curr_y+=(m_font_height+ROW_PIXEL_GAP);

	//ranking (if available)
	int player_rank;
	char player_rank_buffer[64];
	player_rank = (pr->state==STATE_INGAME && !is_dedicated_server)?GetPlayerRankIndex(pr->pnum,player_rank_buffer):-1;
	
	if(player_rank!=-1)
	{
		strcpy(buffer,DTXT_RANK);
		l_width = DLLRenderHUDGetTextLineWidth(buffer);
		if(l_width>max_w) max_w = l_width;
		DLLRenderHUDText(GR_WHITE,m_alpha,0,x_to_use,curr_y,buffer); curr_y+=(m_font_height+ROW_PIXEL_GAP);
	}
	...
	if(player_rank!=-1)
	{
		...
		DLLRenderHUDText(GR_WHITE,m_alpha,0,x_to_use,curr_y,player_rank_buffer); curr_y+=(m_font_height+ROW_PIXEL_GAP);
	}
	...
	for(c=0;c<m_iDColCount;c++){
		...
		if(m_DColInfo[c].type!=DSCOL_BMP){
			...
			//we need to draw a custom bitmap
			(*m_DColCallbackBMP)(m_selected_prec,c,curr_x,curr_y,m_DColInfo[c].width,m_font_height+ROW_PIXEL_GAP,m_alpha);
		}
		...
		if(basethis->FindPInfoStatFirst(m_selected_prec,&stat)){
			if(stat.slot!=m_selected_prec){
				...
				if(dpr && start_index<stats_displayed){
					ddgr_color color;
					color = (dpr->state==STATE_INGAME)?((team_game)?basethis->GetTeamColor(dpr->team):basethis->Player_colors[dpr->pnum]):GR_GREY;
